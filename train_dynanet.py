...
for epoch in range(1, num_epochs + 1):
    ...
    for i, data in tqdm(enumerate(train_data_loader), total=len(train_data_loader), desc=f"Epoch {epoch}/{num_epochs}"):
        ...
        # Forward pass
        data.to(device)
        approach_score_pred, grasp_pred, point_distributions, selected_points, grasp_gt = model(data)
    ...
    with torch.no_grad():
        ...
        for i, val_data in tqdm(enumerate(val_data_loader), total=len(val_data_loader), desc=f"Valid"):
            ...
            val_data.to(device)
            approach_score_pred, grasp_pred, point_distributions, selected_points, val_grasp_gt = model(val_data)
            val_approach_loss, val_grasp_loss = calculate_loss(approach_score_pred, grasp_pred, val_data.approach_scores, val_grasp_gt)
