...
model = GraspNet(global_feat_dim= config.scene_feat_dims, device=device).to(device)
...
for epoch in range(1, num_epochs + 1):
    model.train()
    total_loss = 0
    total_gripper_length_loss = 0
    total_dot_z_x_loss = 0
    ...
    for i, data in tqdm(enumerate(train_data_loader), total=len(train_data_loader), desc=f"Epoch {epoch}/{num_epochs}"):
        ...
        grasp_pred, gripper_lenght, dot_z_x = model(data)
        ...
        total_gripper_length_loss += gripper_length_const.item()
        total_dot_z_x_loss += dot_z_x_const.item()
        if epoch % 50 == 0:
    ...
    average_gripper_length_loss = total_gripper_length_loss / len(train_data_loader)
    average_dot_z_x_loss = total_dot_z_x_loss / len(train_data_loader)
    wandb.log({"Train Loss": average_loss}, step=epoch)
    wandb.log({"Gripper Length Loss": average_gripper_length_loss}, step=epoch)
    wandb.log({"Dot Z X Loss": average_dot_z_x_loss}, step=epoch)
    ...
    with torch.no_grad():
        ...
        for i, val_data in tqdm(enumerate(val_data_loader), total=len(val_data_loader), desc=f"Valid"):
            ...
            grasp_pred, gripper_lenght, dot_z_x = model(val_data)
            ...
            if epoch % 50 == 0:
                ...
                for j in range(preds.shape[0]):
                    grasp_pred = preds[j]
                    valid_grasp_success += check_grasp_success_from_dict(grasp_pred, val_data[j].sample_info,  0.03, np.deg2rad(30))
